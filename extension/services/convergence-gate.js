...
function createGateFinalizer(opts, emit) {
    return (result) => {
...
function selectWorkspaceTargetDirs(opts, workspacePackages) {
...
function selectGateTargetDirs(opts, emit, empty, start) {
...
async function detectWorkingDirDrift(opts, emit, start, allowedPathsUsed) {
...
async function shouldSkipUnsafeTest(check, projectType, dir, emit) {
...
async function runTimedCheck(cmd, dir, check, effectiveMs, emit) {
...
async function runGateChecks(opts, projectType, cmdMap, targetDirs, emit) {
    ...
    outerLoop: for (const dir of targetDirs) {
        for (const check of opts.checks) {
            ...
            if (!cmd)
                continue;
            if (await shouldSkipUnsafeTest(check, projectType, dir, emit))
                continue;
...
async function handleBaselineMode(opts, projectType, realFailures, allowedPathsUsed, start, emit) {
...
async function handleKnownFlakes(opts, realFailures, allFailures, flakeFailures, allowedPathsUsed, start, emit) {
...
function buildStrictGateResult(realFailures, allFailures, allowedPathsUsed, start, emit) {
