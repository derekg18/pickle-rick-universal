...
export function parseGeminiTokenUsageFile(filePath) {
    const content = readTextFile(filePath);
    if (content === null)
        return resultWithUnknown('gemini', filePath, 'Gemini log missing or unreadable');
    return parseGeminiTokenUsage(content, filePath);
}
export function parseGeminiTokenUsage(content, sourcePath) {
    const records = [];
