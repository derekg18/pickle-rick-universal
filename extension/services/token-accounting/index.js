...
import { notifySessionEvent } from '../notification-dispatcher.js';
...
function listCandidateFiles(sessionDir) {
    const files = [];
    function walk(dir, depth) {
        ...
        for (const entry of entries) {
            ...
            if (!entry.isFile())
                continue;
            if (entry.name === ARTIFACT_JSON || entry.name === ARTIFACT_MARKDOWN)
                continue;
            if (!CANDIDATE_EXTENSIONS.has(path.extname(entry.name)))
                continue;
            try {
                if (fs.statSync(fullPath).size > MAX_SCAN_BYTES)
                    continue;
            }
            catch {
                continue;
...
function likelyBackendForPath(filePath, backend) {
    return path.basename(filePath).toLowerCase().includes(backend);
}
function collectRecords(sessionDir, backend) {
...
function summarizeConfidence(records) {
    if (records.length === 0)
        return 'unknown';
    return records.reduce((lowest, record) => (confidenceRank(record.cost_confidence) < confidenceRank(lowest) ? record.cost_confidence : lowest), 'exact');
}
...
function valueOrUnknown(value, digits) {
    if (value === null)
        return 'unknown';
    return typeof digits === 'number' ? value.toFixed(digits) : String(value);
}
export function renderTokenAccountingMarkdown(summary) {
    const lines = [
        '# Token Accounting',
        '',
...
export function buildTokenUsageSummary(sessionDir, records, jsonPath, markdownPath, now = new Date()) {
    let pricingUpdatedAt = null;
    const pricedRecords = records.map((record) => {
        const pricing = estimateRecordCost(record, now);
        if (pricing.pricing_table_updated_at && !pricingUpdatedAt)
            pricingUpdatedAt = pricing.pricing_table_updated_at;
...
export function writeTokenAccountingReports(sessionDir, opts = {}) {
...
export function finalizeTokenAccounting(sessionDir, opts = {}) {
    try {
