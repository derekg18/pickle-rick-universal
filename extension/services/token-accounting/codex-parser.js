...
export function parseCodexTokenUsageFile(filePath) {
    const content = readTextFile(filePath);
    if (content === null)
        return resultWithUnknown('codex', filePath, 'Codex log missing or unreadable');
    return parseCodexTokenUsage(content, filePath);
}
export function parseCodexTokenUsage(content, sourcePath) {
    const records = [];
