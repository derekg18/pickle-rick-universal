...
function isPerformanceFrontendCommand(value) {
...
function appendViewportRows(value, viewportRows) {
    for (const viewport of value.split(',')) {
        const parsed = parseViewport(viewport);
        if (!parsed)
            return invalidViewportResult(viewport);
...
export function parsePerformanceFrontendArgs(args) {
    const state = {
        appPath: '.',
        target: null,
    ...
    for (let index = 0; index < args.length; index++) {
        ...
        if (!args[index].startsWith('--'))
            state.positional.push(args[index]);
    }
    if (!state.target && state.positional.length > 0)
        state.target = state.positional.join(' ');
    return {
...
function consumeAppArg(args, index, state) {
...
function consumeTargetArg(args, index, state) {
    const arg = args[index];
...
function fixtureChangedFiles(ctx) {
    return ctx.changedFiles ?? [];
}
function buildSet(changedFiles) {
    const scopes = new Set();
    for (const file of changedFiles) {
        if (file.startsWith('src/') || file.startsWith('app/') || file.startsWith('pages/'))
            scopes.add('app');
        if (file.includes('package.json') || file.includes('vite.config') || file.includes('next.config'))
            scopes.add('build-config');
        if (file.endsWith('.css') || file.endsWith('.scss'))
            scopes.add('styles');
    }
    return [...scopes].sort();
}
function skippedSet(changedFiles) {
    const build = new Set(buildSet(changedFiles));
    return ['app', 'build-config', 'styles'].filter((scope) => !build.has(scope));
}
export function runPerformanceFrontendCommand(command, args = [], ctx = {}) {
...
export function runPerformanceFrontendCommandByName(command, args = [], ctx = {}) {
