...
- `team-flow-profile.ts` — INVARIANT: skip classifiers cannot bypass prior phase input artifacts. BREAKS: full team-flow accepts missing review artifacts before security/QA. ENFORCE: `security skip still requires review artifacts from the previous phase`; `qa gate requires review artifacts when deterministic classifier replaces security output`. PATTERN_SHAPE: `canSkip.*deterministic_no_sensitive_paths` before `missingArtifacts` evaluation.
- `metrics-utils.ts` — INVARIANT: report grouping must match row granularity. BREAKS: weekly JSON advertises buckets while emitting daily rows. ENFORCE: `buildReport: weekly grouping aggregates daily rows into ISO week buckets`. PATTERN_SHAPE: `grouping === 'weekly'` without `aggregateWeeklyRows(rows)`.
...
- INVARIANT: `flags` stores optional feature flags with typed known keys (`strict_teams`, `human_help_requested`, `human_help_reason`, `human_help_recovery_command`, `jerry_mode_signature`, `os_notifier_available`) and unknown-key preservation. ENFORCE: extension/tests/state-field-invariants.test.js.
