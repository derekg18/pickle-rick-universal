...
export type GeneratorCommandStatus = 'success' | 'failed' | 'needs_followup';
...
export interface GeneratorCommandContext {
  workspaceDir?: string;
  now?: Date;
  tempDir?: string;
}

export interface GeneratorCommandArtifact {
...
export interface GeneratorCommandResult {
  command: GeneratorCommand;
  status: GeneratorCommandStatus;
  summary: string;
  code?: GeneratorErrorCode;
  remediation?: string;
  artifact: GeneratorCommandArtifact;
  plan?: GeneratorPlan;
}
...
export interface MockSeedPlan {
  role: 'mock-seed-preview';
  schema_name: string;
  rows: SeedRow[];
  referential_checks: string[];
}
...
export interface MigrationOperation {
  action: 'create_table' | 'drop_table' | 'add_column' | 'drop_column' | 'alter_column';
  table: string;
  column?: string;
  detail: string;
}
...
export interface ScaffoldPlan {
  role: 'scaffold-plan';
  kind: Exclude<GeneratorArtifactKind, 'mock-seed-preview' | 'migration-plan' | 'retry-plan'>;
  name: string;
  files: ScaffoldFile[];
...
const COMMANDS: readonly GeneratorCommand[] = [
  'ricks-flask',
  'morty-smith-database',
  'unity',
  'gazorpazorp',
...
function isGeneratorCommand(value: string): value is GeneratorCommand {
  return (COMMANDS as readonly string[]).includes(value);
}

function defaultSchema(name = 'fixture'): ParsedSchema {
  return {
    name,
    tables: [
      {
...
function parseAttempts(value: string): number {
  ...
  if (!Number.isSafeInteger(attempts) || attempts < 1) return 3;
...
function targetDir(parsed: ParsedArgs, ctx: GeneratorCommandContext, fallbackName: string): string {
  return parsed.targetDir ?? path.join(ctx.workspaceDir ?? 'workspace', fallbackName);
}

function previewDir(ctx: GeneratorCommandContext, command: GeneratorCommand): string {
  const root = ctx.tempDir ?? path.join(ctx.workspaceDir ?? 'workspace', '.pickle-preview');
  return path.join(root, command);
}

function planPath(artifact: Omit<GeneratorCommandArtifact, 'plan_path'>): string {
...
function baseArtifact(
  command: GeneratorCommand,
  parsed: ParsedArgs,
  ctx: GeneratorCommandContext,
  fallbackName: string,
): GeneratorCommandArtifact {
  const artifact = {
    kind: 'generator-command-result' as const,
    command,
...
function confirmationRequired(
  ...
  ctx: GeneratorCommandContext,
  fallbackName: string,
): GeneratorCommandResult | null {
  if (!parsed.unsafeRequested || parsed.confirmed) return null;
  return {
    command,
...
function targetDirFailure(
  ...
  if (parsed.force || !fs.existsSync(target)) return null;
  const stat = fs.statSync(target);
  if (stat.isDirectory() && fs.readdirSync(target).length === 0) return null;
  return failed(
    command,
    parsed,
    ctx,
    fallbackName,
    'TARGET_DIR_NOT_EMPTY',
    `/${command} refused to scaffold into a non-empty target directory.`,
    'Choose an empty target directory, omit the target to use preview output, or re-run with --force after reviewing the plan.',
  );
}
...
function readSchemaInput(input: string | null, fallbackName: string): ParsedSchema {
  if (!input) return defaultSchema(fallbackName);
  const raw = fs.existsSync(input) ? fs.readFileSync(input, 'utf8') : input;
  const value = JSON.parse(raw) as unknown;
  return normalizeSchema(value, fallbackName);
}

function normalizeSchema(value: unknown, fallbackName: string): ParsedSchema {
  if (!isRecord(value)) throw new Error('schema root must be an object');
  ...
  if (isRecord(value.properties)) {
    return {
      ...
      tables: Object.entries(value.properties).map(([tableName, tableValue]) => {
        if (!isRecord(tableValue)) return { name: tableName, columns: { id: 'string' } };
...
function normalizeTable(value: unknown): TableShape {
  if (!isRecord(value) || typeof value.name !== 'string') throw new Error('table entries require a string name');
...
function normalizeColumns(columns: Record<string, unknown>): Record<string, string> {
  return Object.fromEntries(
    Object.entries(columns).map(([name, value]) => [name, columnType(value)]),
  );
}

function columnType(value: unknown): string {
...
function schemaFailure(
  ...
  return failed(
    ...
    fallbackName,
    'INVALID_SCHEMA',
    `/${command} could not parse the supplied schema fixture.`,
    error instanceof Error ? error.message : 'Provide JSON schema input or a readable schema file path.',
  );
}

function buildMockRows(schema: ParsedSchema): SeedRow[] {
  return schema.tables.map((table) => ({
    table: table.name,
    values: Object.fromEntries(
      Object.entries(table.columns).map(([column, type]) => [column, sampleValue(column, type)]),
    ),
  }));
}

function sampleValue(column: string, type: string): string | number | boolean | null {
  if (column === 'id') return `${column}-1`;
  if (column.endsWith('_id')) return `${column.slice(0, -3)}-1`;
...
function diffSchemas(oldSchema: ParsedSchema, newSchema: ParsedSchema): MigrationOperation[] {
  ...
  for (const [name] of newTables) {
    if (!oldTables.has(name)) operations.push({ action: 'create_table', table: name, detail: `Create table ${name}.` });
  }
  for (const [name] of oldTables) {
    if (!newTables.has(name)) operations.push({ action: 'drop_table', table: name, detail: `Drop table ${name}.` });
  }
...
function scaffoldResult(
  ...
  return success(
    ...
    ctx,
    fallbackName,
    `Prepared ${kind} preview for ${name}.`,
    {
      role: 'scaffold-plan',
      kind,
      name,
...
function runRicksFlask(args: readonly string[], ctx: GeneratorCommandContext): GeneratorCommandResult {
  ...
  try {
    ...
    return success(
      ...
      ctx,
      'mock-seed-preview',
      `Prepared referential mock seed preview for ${schema.name}.`,
      {
        role: 'mock-seed-preview',
...
function runMortySmithDatabase(args: readonly string[], ctx: GeneratorCommandContext): GeneratorCommandResult {
  const parsed = parseArgs(args);
  ...
  try {
    ...
    return success(
      ...
      ctx,
      'migration-plan',
      `Prepared migration plan from ${oldSchema.name} to ${newSchema.name}.`,
      {
        role: 'migration-plan',
...
function runUnity(args: readonly string[], ctx: GeneratorCommandContext): GeneratorCommandResult {
  const parsed = parseArgs(args);
...
function runGazorpazorp(args: readonly string[], ctx: GeneratorCommandContext): GeneratorCommandResult {
  const parsed = parseArgs(args);
...
function runButterRobot(args: readonly string[], ctx: GeneratorCommandContext): GeneratorCommandResult {
  const parsed = parseArgs(args);
...
function runSnakeJazz(args: readonly string[], ctx: GeneratorCommandContext): GeneratorCommandResult {
  const parsed = parseArgs(args);
...
export function runGeneratorCommandGroup(
  command: GeneratorCommand,
...
export function runGeneratorCommandGroupByName(
  command: string,
  args: readonly string[] = [],
  ctx: GeneratorCommandContext = {},
): GeneratorCommandResult {
  if (!isGeneratorCommand(command)) {
    ...
    return failed(
      ...
      'generator-preview',
      'UNKNOWN_COMMAND',
      `Unknown generator command: ${command}`,
