import type { TokenCostConfidence, TokenUsageRecord } from './types.js';
...
export const DEFAULT_PRICING_TABLE: readonly PricingEntry[] = [
...
export function isPricingEntryStale(entry: PricingEntry, now = new Date()): boolean {
...
export function estimateRecordCost(
  ...
  now = new Date(),
  table: readonly PricingEntry[] = DEFAULT_PRICING_TABLE,
): PricingResult {
  if (!record.model) {
