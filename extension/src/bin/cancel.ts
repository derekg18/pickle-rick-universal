...
function deactivateSessionState(statePath: string): boolean {
  try {
...
function removeSessionMapEntry(sessionsMapPath: string, cwd: string): void {
  let freshMap: Record<string, unknown> = {};
  try {
    freshMap = (readRecoverableJsonObject(sessionsMapPath) || {}) as Record<string, unknown>;
  } catch { /* ignore */ }
...
function cancelWithSessionMapLock(sessionsMapPath: string, statePath: string, cwd: string): boolean {
  let cancelled = false;
...
function cancelWithoutSessionMapConsistency(statePath: string, err: LockError): boolean {
...
function printCancelOutcome(cancelled: boolean, sessionPath: string): void {
  if (cancelled) {
    printMinimalPanel(
      'Loop Cancelled',
      {
