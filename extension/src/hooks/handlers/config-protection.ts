...
function resolveHookPrecheckBackend(state: { backend?: unknown }): Backend {
  if (typeof state.backend === 'string' && (BACKENDS as readonly string[]).includes(state.backend)) {
    return state.backend as Backend;
  }

  const envBackend = process.env.PICKLE_BACKEND;
  if (typeof envBackend === 'string' && (BACKENDS as readonly string[]).includes(envBackend)) {
    return envBackend as Backend;
  }

  return 'claude';
}

function supportsHookPrecheck(backend: Backend): boolean {
  return backend === 'claude';
}

function isWorkerSpawnPrecheck(input: PreToolUseInput): boolean {
  return input.tool_name === 'Bash'
    && typeof input.tool_input?.command === 'string'
    && /\bspawn-morty\.js\b/.test(input.tool_input.command);
}

function currentTicketFile(sessionDir: string, ticketId: string): string | null {
  try {
...
function codexRequiredWorkerBlockReason(
  sessionDir: string,
  state: { backend?: unknown; current_ticket?: string | null },
): string | null {
  const backend = resolveHookPrecheckBackend(state);
  if (!supportsHookPrecheck(backend)) return null;
  if (!state.current_ticket) return null;
  ...
  if (!ticket?.codex_required) return null;
