...
function isInvalidHookName(hookName) {
    return hookName.includes('/') || hookName.includes('\\') || hookName.includes('..');
}
function resolveHandlerCommand(hookName, extraArgs) {
...
function parseDecision(stdout, hookName) {
...
function handleChildClose(hookName, code, stdout, stderr) {
    if (stderr)
...
function runHandler(command, hookName, inputData) {
    try {
