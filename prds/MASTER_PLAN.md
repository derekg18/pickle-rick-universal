...
| **`prds/anatomy-park-finalizer-history-crash.md`** | **Bug PRD** (2026-04-30) — Surfaced live during `pipeline-a5e02f01`. **Successful worker-managed convergence followed by FATAL `Cannot read properties of undefined (reading 'history')`.** Fixed with defensive guards in `microverse-runner.ts`, state preservation in `markMicroverseFatalError`, and a new unit test suite. | committed `59b2c8a` |
